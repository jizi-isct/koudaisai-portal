...
#[instrument(name = "init /plans_info/v1")]
pub fn init_service<S: Clone + 'static, E: From<std::convert::Infallible> + 'static>(
    secrets: Secrets,
) -> MethodRouter<S, E> {
    // 上流ホスト（:authority）を指定。
    let upstream = axum_proxy::builder_https("events26.koudaisai.jp").expect("build proxy client");

    // /api/plans_info/{*path} の先頭1回だけ /v1 に置換
    // 例: /api/plans_info/plans?x=1 → /v1/plans?x=1
    let host = HeaderValue::from_static("events26.koudaisai.jp");
    let cf_id =
