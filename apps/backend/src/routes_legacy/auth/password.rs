...
async fn post_change(
    ...
    // TODO(sqlx移行): users の取得・パスワード検証・更新を application 層(sqlx)へ再配線する。
    // JWT デコード/検証は温存対象だが、user(UserRead) 取得が DB 依存のため本体をスタブ化。
...
async fn post_reset(
    ...
    // TODO(sqlx移行): users の検索・パスワードリセットメール送信を application 層(sqlx)へ再配線する。
    // user.id を用いた issue_password_reset_token(JWT発行)も再配線時に復活させる。
...
async fn post_reset_confirm(
    ...
    // TODO(sqlx移行): users の検索・パスワード変更(change_password)を application 層(sqlx)へ再配線する。
    // パスワードリセットトークンのデコード/エラーハンドリングは温存対象だが、
    // user(UserRead) 取得が DB 依存のため本体をスタブ化。
