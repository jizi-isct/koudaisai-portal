...
impl GroupRepo<MemoryTransaction> for MemoryGroupRepo {
    async fn find_by_id(&self, id: GroupId) -> Result<Option<Group>, FindError> {
        let groups = self.groups.read().map_err(|e| FindError::InternalError(anyhow!(e.to_string())))?;
        Ok(groups.get(&id).map(|g| Group::restore(g.id(), g.created_at(), g.updated_at(), g.name().to_string(), g.r#type().clone()).unwrap()))
    }

    async fn find_all(&self) -> Result<Vec<Group>, FindError> {
        let groups = self.groups.read().map_err(|e| FindError::InternalError(anyhow!(e.to_string())))?;
        Ok(groups.values().map(|g| Group::restore(g.id(), g.created_at(), g.updated_at(), g.name().to_string(), g.r#type().clone()).unwrap()).collect())
    }

    async fn insert(&self, group: Group) -> Result<(), InsertError> {
        let mut groups = self.groups.write().map_err(|e| InsertError::InternalError(anyhow!(e.to_string())))?;
    ...
    async fn insert_in(&self, _tx: &mut MemoryTransaction, group: Group) -> Result<(), InsertError> {
        self.insert(group).await
    }

    async fn update(&self, group: Group) -> Result<(), UpdateError> {
        let mut groups = self.groups.write().map_err(|e| UpdateError::InternalError(anyhow!(e.to_string())))?;
        if !groups.contains_key(&group.id()) {
    ...
    async fn update_in(&self, _tx: &mut MemoryTransaction, group: Group) -> Result<(), UpdateError> {
        self.update(group).await
    }

    async fn delete(&self, id: GroupId) -> Result<(), DeleteError> {
        let mut groups = self.groups.write().map_err(|e| DeleteError::InternalError(anyhow!(e.to_string())))?;
