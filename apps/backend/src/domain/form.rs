...
mod tests {
    ...
    fn test_register_success() {
        ...
        assert_eq!(form.updated_by(), created_by);
        assert_eq!(form.created_at(), clock.now);
        assert_eq!(form.updated_at(), clock.now);
    }
    ...
    #[test]
    fn test_register_empty_summary() {
        let clock = MockClock { now: Utc::now() };
        let created_by = Uuid::new_v4();
        let targets = vec![TargetSpecifier::GroupTypePress];
        let name = "Test Form".to_string();
    ...
    #[test]
    fn test_register_whitespace_summary() {
        let clock = MockClock { now: Utc::now() };
        let created_by = Uuid::new_v4();
        let targets = vec![TargetSpecifier::GroupTypePress];
        let name = "Test Form".to_string();
    ...
    fn test_rename_success() {
        ...
        let new_name = "New Form Name".to_string();
        let updated_by = Uuid::new_v4();
        let update_time = initial_time + chrono::Duration::seconds(10);
        let clock_updated = MockClock { now: update_time };

        let result = form.rename(new_name.clone(), updated_by, &clock_updated);
        assert!(result.is_ok());
        assert_eq!(form.name(), new_name);
        assert_eq!(form.updated_by(), updated_by);
        assert_eq!(form.updated_at(), update_time);
        assert_eq!(form.created_at(), initial_time);
    }
    ...
    fn test_rename_empty_name() {
        ...
        let result = form.rename("".to_string(), Uuid::new_v4(), &clock);
        assert!(matches!(result, Err(FactoryError::InvalidInput(_))));
    }

    #[test]
    fn test_rename_whitespace_name() {
        let clock = MockClock { now: Utc::now() };
        let mut form = setup_form(&clock);
    ...
    #[test]
    fn test_change_summary_success() {
        let initial_time = Utc::now();
        let clock = MockClock { now: initial_time };
        let mut form = setup_form(&clock);

        let new_summary = "New Summary".to_string();
        let updated_by = Uuid::new_v4();
        let update_time = initial_time + chrono::Duration::seconds(10);
        let clock_updated = MockClock { now: update_time };

        let result = form.change_summary(new_summary.clone(), updated_by, &clock_updated);
        assert!(result.is_ok());
        assert_eq!(form.summary(), new_summary);
        assert_eq!(form.updated_by(), updated_by);
        assert_eq!(form.updated_at(), update_time);
        assert_eq!(form.created_at(), initial_time);
    }
    ...
    fn test_change_summary_empty() {
        ...
        let result = form.change_summary("".to_string(), Uuid::new_v4(), &clock);
        assert!(matches!(result, Err(FactoryError::InvalidInput(_))));
    }

    #[test]
    fn test_change_summary_whitespace() {
        let clock = MockClock { now: Utc::now() };
        let mut form = setup_form(&clock);
