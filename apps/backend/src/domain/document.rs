...
mod tests {
    ...
    fn test_register_success() {
        ...
        let category = Some(Uuid::new_v4());
        let document_format = DocumentFormat::Markdown {
            content: "test content".to_string(),
        };
        let created_by = UserId::new(Uuid::new_v4());

        let document = Document::register(
            title.to_string(),
            category,
            document_format,
            vec![TargetSpecifier::UserId(UserId::new(Uuid::new_v4()))],
            created_by,
            &clock,
        )
        .unwrap();
        ...
        assert_eq!(document.created_at(), clock.now);
        assert_eq!(document.updated_at(), clock.now);
    }
