...
pub fn test_new(_path: &Path, contents: String) -> datatest_stable::Result<()> {
    ...
    if c.ok {
        let id = result
            .unwrap_or_else(|_| panic!("expected Ok for {}:{}", c.prefix, c.index));
        if let Some(exp) = &c.display {
            assert_eq!(id.to_string(), *exp, "display mismatch for {}:{}", c.prefix, c.index);
        }
        if let Some(exp) = &c.index_str {
            assert_eq!(id.index_str(), *exp, "index_str mismatch for {}:{}", c.prefix, c.index);
        }
    } else {
