...
use events26_api::models::GetProjectDetails200ResponseMenu;
...
fn sample_menu() -> GetProjectDetails200ResponseMenu {
    GetProjectDetails200ResponseMenu {
        description: "販売メニュー".to_string(),
        items: vec![
            events26_api::models::GetProjectDetails200ResponseMenuItemsInner {
                name: "豚汁".to_string(),
                price: Some(100.0),
                options: vec![
                    events26_api::models::GetProjectDetails200ResponseMenuItemsInnerOptionsInner {
                        name: "大盛り".to_string(),
                        price: Some(50.0),
                    },
                ],
...
#[derive(Deserialize)]
struct MenuUpdateCase {
    actor: ActorSpec,
    menu: GetProjectDetails200ResponseMenu,
    expected: String,
}

pub fn test_update_own_project_menu(_path: &Path, contents: String) -> datatest_stable::Result<()> {
    crate::application::common::run(async {
        let c: MenuUpdateCase = serde_json::from_str(&contents)?;
        let api = MemoryEvents26Api::new();
        let app = Events26App::new(&api);
        let (_, actor) = build_actor(c.actor);

        let result = app.update_own_project_menu(&actor, &c.menu).await;

        match c.expected.as_str() {
            "ok" => {
                result.expect("expected update Ok");
                assert_eq!(api.menu(PROJECT_ID), Some(c.menu));
            }
            "unauthorized" => {
                assert!(matches!(
                    result,
                ...
                assert_eq!(api.menu(PROJECT_ID), None);
            }
            expected => panic!("unknown expected: {expected}"),
        }
        Ok(())
...
pub fn test_delete_own_project_menu(_path: &Path, contents: String) -> datatest_stable::Result<()> {
    crate::application::common::run(async {
        let c: MenuDeleteCase = serde_json::from_str(&contents)?;
        let api = MemoryEvents26Api::new();
        if c.menu_exists {
            api.update_project_menu(PROJECT_ID, &sample_menu())
                .await
                .unwrap();
        }
        ...
        let app = Events26App::new(&api);
        let (_, actor) = build_actor(c.actor);
        let result = app.delete_own_project_menu(&actor).await;
        ...
        match c.expected.as_str() {
            ...
            "unauthorized" => {
                ...
                assert_eq!(api.menu(PROJECT_ID), c.menu_exists.then(sample_menu));
            }
            expected => panic!("unknown expected: {expected}"),
        }
        Ok(())
