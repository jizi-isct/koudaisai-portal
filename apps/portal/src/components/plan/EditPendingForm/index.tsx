...
export function EditPendingForm({ refetch, planId, approvalRequest }: Props) {
  const { mutateAsync: closeApprovalRequest } = $apiMembers.useMutation(
    "post",
    "/users/{user_id}/approval_requests/{request_id}/close"
  );
  const [isClosing, setIsClosing] = React.useState<boolean>(false);

  const { data: plan } = $plansInfoApiNoLogin.useQuery(
    "get",
    "/plans/{planId}",
    {
      params: {
