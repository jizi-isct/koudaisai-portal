  ...
  if (!token) {
    return (
      <section className={styles.root}>
        ...
        <p className={styles.description}>tokenが指定されていません。URLに?token=xxxxのように指定してください。</p>
      </section>
    );
  }

  return (
    <form className={styles.root} action="/reset-password/confirm/" method="post" onSubmit={handleSubmit}>
      ...
      <NextPhaseButton type="submit" label="パスワードをリセット" disabled={isSubmitting} aria-busy={isSubmitting} />
    </form>
  );
}
