    ...
    case 'password':
      return (
        <ActivationPhasePassword
          mAddress={mAddress}
          next={handlePasswordChange}
        />
      );
    case 'activating':
      return (
        <ActivationPhaseActivating
          mAddress={mAddress}
          password={password}
          token={token}
          next={() => setPhase('done')}
  ...
  return (
    ...
    >
      <img
        ...
        width={size === 'small' ? 40 : 300}
        height={size === 'small' ? 40 : 300}
      />
    </div>
  );
}
...
function ActivationPhaseActivating({
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      switch (response.status) {
        case 401:
          setError(
            '有効化トークンあるいはmアドレスが無効です。もう一度やり直してください。',
          );
          break;
        case 404:
  ...
  return (
    <main className={formStyles.root}>
      ...
      <div
        className={`${styles.spinner} ${styles.item}`}
        aria-label="読み込み中"
      />
    </main>
  );
