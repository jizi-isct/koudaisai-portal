...
function UserTable(){
    ...
    const targetedUsers = userDataSet.filter((item) => item.name.includes(keyWords));
            ...
            render: (_value, record) => {
                return (
                    <a
                        ...
                        href={`/manageUsers/view?user_id=${record.id}`}
                    >
                        {record.name}
                    </a>
                );
    ...
    return(
        <Flex gap={8} vertical>
            <Input placeholder="名前を検索" onChange={handleOnChange} style={{ width: 200 }} value={keyWords}/>
            <Table<UserRead>
