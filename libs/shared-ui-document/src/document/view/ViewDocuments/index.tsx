...
export function ViewDocuments({ documents, download }: ViewDocumentsProps) {
  return documents.map(({ category, documents }, i) => (
    <section key={`document-category-${i}`}>
      {category && (
        <HeadingViewDocumentCategory
          documentCategory={category}
          defaultEmoji={headingEmojis[i % headingEmojis.length]}
        />
      )}

      <ContentList
        contents={documents.map((document, j) => (
          <ContentRowViewDocument
            key={`document-${i}.${j}`}
            download={() => download(document.id)}
            document={document}
          />
        ))}
      />
